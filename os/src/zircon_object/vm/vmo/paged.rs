use {
    super::*,
    super::util::block_range::BlockIter,
    ...
    core::sync::atomic::*,
    hashbrown::HashMap,
    crate::kernel_hal::{frame_flush, PhysFrame, PAGE_SIZE},
...
impl VMObjectTrait for VMObjectPaged {
    fn read(&self, offset: usize, buf: &mut [u8]) -> ZxResult {
        ...
        inner.for_each_page(offset, buf.len(), MMUFlags::READ, |paddr, buf_range| {
            crate::kernel_hal::pmem_read(paddr, &mut buf[buf_range]);
        })
    }
    ...
    fn write(&self, offset: usize, buf: &[u8]) -> ZxResult {
        ...
        inner.for_each_page(offset, buf.len(), MMUFlags::WRITE, |paddr, buf_range| {
            crate::kernel_hal::pmem_write(paddr, &buf[buf_range]);
        })
    }
    ...
    fn zero(&self, offset: usize, len: usize) -> ZxResult {
        ...
        for block in iter {
            ...
            if block.len() == PAGE_SIZE && !inner.is_contiguous() {
                ...
                crate::kernel_hal::frame_zero_in_range(paddr, block.begin, block.end);
            }
        }
...
impl VMObjectPaged {
    ...
    pub fn create_contiguous(&self, size: usize, align_log2: usize) -> ZxResult {
        ...
        for (i, f) in frames.drain(0..).enumerate() {
            crate::kernel_hal::frame_zero_in_range(f.addr(), 0, PAGE_SIZE);
    ...
    fn commit_page_internal(
        ...
        if no_frame {
            ...
            if out_of_range || no_parent {
                ...
                crate::kernel_hal::frame_zero_in_range(target_frame.addr(), 0, PAGE_SIZE);
                if out_of_range {
        ...
        if frame.tag.is_split() {
            ...
            // copy-on-write
            let target_frame = PhysFrame::alloc().ok_or(ZxError::NO_MEMORY)?;
            crate::kernel_hal::frame_copy(frame.frame.addr(), target_frame.addr());
            frame.tag = child_tag;
            return Ok(CommitResult::CopyOnWrite(target_frame));
        }
