...
fn main() {
    println!("cargo:rerun-if-changed=linux_syscall/syscall.h.in");

    let mut fout = std::fs::File::create("./consts.rs").unwrap();
    ...
    let data = std::fs::read_to_string("./syscall.h.in").unwrap();
